...
namespace PixelCrushers.QuestMachine
{
    ...
    public class QuestConditionSet
    {
        ...
        /// Note: Does not reset numTrueConditions. This method is used to block quest offers if a quest was
        /// initially marked as offerable but then the offer conditions became false later.
        /// </summary>
        public bool RecheckConditions()
        {
            //--- We no longer short-circuit evaluation since new circumstances
            //--- may make some conditions true.
            int numTrue = 0;
            foreach (var condition in conditionList)
            {
                var wasConditionTrue = condition.alreadyTrue;
                var isTrueNow = condition.Recheck();
                if (wasConditionTrue && !isTrueNow)
                {
                    numTrueConditions = Mathf.Max(0, numTrueConditions - 1);
                }
                else if (isTrueNow)
                {
                    numTrue++;
            ...
            numTrueConditions = numTrue;
            switch (conditionCountMode)
            {
                default:
                case ConditionCountMode.All: return numTrue >= conditionList.Count;
                case ConditionCountMode.Any: return numTrue >= 1 || conditionList.Count == 0;
                case ConditionCountMode.Min: return numTrue >= minConditionCount;
            }
        }
