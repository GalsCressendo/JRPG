...
namespace PixelCrushers.QuestMachine
{
    ...
    public class UnityUIQuestJournalUI : UnityUIBaseUI, IQuestJournalUI
    {
        ...
        protected override void InitializeTemplates()
        {
            ...
            if (subheadingTemplates != null)
            {
                foreach (var subheadingTemplate in subheadingTemplates)
                {
                    if (subheadingTemplate != null) subheadingTemplate.gameObject.SetActive(false);
                }
            }
        ...
        protected virtual void RefreshNow() 
        {
            ...
            selectionPanelContentManager.RetireInstances();
            ...
            selectionPanelContentManager.DespawnRetiredInstances();
        ...
        protected void AddQuestGroupToUI(List<Quest> quests, string groupName)
        {
            ...
            var groupFoldout = selectionPanelContentManager.AcquireContentInstance<UnityUIFoldoutTemplate>(questGroupTemplate, groupName, null);
            selectionPanelContentManager.AddToContainer(groupFoldout, questSelectionContentContainer);
        ...
        protected virtual void AddGrouplessQuestsToUI()
        {
            foreach (var quest in questJournal.questList)
            {
                ...
                var template = GetQuestNameTemplateForState(quest.GetState());
                var questName = selectionPanelContentManager.AcquireContentInstance<UnityUIQuestNameButtonTemplate>(template, quest, "groupless");
                questName.Assign(quest, OnToggleTracking);
                selectionPanelContentManager.AddToContainer(questName, questSelectionContentContainer);
                var target = quest;
        ...
        protected virtual void AddQuestToUI(Quest quest, RectTransform container)
        {
            ...
            var template = GetQuestNameTemplateForState(quest.GetState());
            var questName = selectionPanelContentManager.AcquireContentInstance<UnityUIQuestNameButtonTemplate>(template, quest, null);
            questName.Assign(quest, OnToggleTracking);
            selectionPanelContentManager.AddToContainer(questName, container);
            var target = quest;
            SetupQuestNameUIEvents(questName.buttonTemplate.button, target);
        ...
        protected virtual void RepaintSelectedQuest()
        {
            ...
            detailsPanelContentManager.RetireInstances();
            ...
            detailsPanelContentManager.DespawnRetiredInstances();
            isDrawingSelectionPanel = true;
