...
namespace PixelCrushers.QuestMachine
{
    ...
    public abstract class UnityUIBaseUI : MonoBehaviour
    {
        ...
        {
            if (QuestMachine.isQuitting) return;
            if (gameObject == null) return; // GameObject may have been destroyed in same frame if leaving Unity editor's play mode.
        ...
        protected virtual void SetContents(string displayName, Sprite image, List<QuestContent> contents)
        {
            ...
            currentContentManager.RetireInstances();
            currentIconList = null;
            currentButtonList = null;
        ...
        {
            var template = GetHeadingTemplate(headingContent.headingLevel);
            var instance = currentContentManager.AcquireContentInstance<UnityUITextTemplate>(template, headingContent, null);
            instance.Assign(headingContent.runtimeText, headingContent.setColor, headingContent.color);
            currentContentManager.AddToContainer(instance, currentContentContainer);
            currentIconList = null;
        }

        protected virtual void AddBodyContent(BodyTextQuestContent bodyContent)
        {
            var instance = currentContentManager.AcquireContentInstance<UnityUITextTemplate>(currentBodyTemplate, bodyContent, null);
            instance.Assign(bodyContent.runtimeText, bodyContent.setColor, bodyContent.color);
            currentContentManager.AddToContainer(instance, currentContentContainer);
            currentIconList = null;
        }
        ...
        {
            var instance = currentContentManager.AcquireContentInstance<UnityUITextTemplate>(currentBodyTemplate, bodyContentString, null);
            instance.text.text = bodyContentString;
            currentContentManager.AddToContainer(instance, currentContentContainer);
        ...
        {
            var iconList = currentContentManager.AcquireContentInstance<UnityUIIconListTemplate>(currentIconListTemplate, null, null);
            iconList.contentManager = currentContentManager;
            currentContentManager.AddToContainer(iconList, currentContentContainer);
            return iconList;
        }
        ...
        {
            var buttonList = currentContentManager.AcquireContentInstance<UnityUIButtonListTemplate>(currentButtonListTemplate, null, null);
            buttonList.contentManager = currentContentManager;
            currentContentManager.AddToContainer(buttonList, currentContentContainer);
            return buttonList;
        }
