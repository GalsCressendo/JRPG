...
namespace PixelCrushers.QuestMachine
{
    ...
    public class QuestEditorWindow : EditorWindow
    {
        ...
        // Entity ID of the current quest asset.
        [SerializeField]
        private EntityIdWrapper m_questEntityId;
        ...
        // Entity ID of the current QuestListContainer, or 0 if none.
        [SerializeField]
        private EntityIdWrapper m_questListContainerEntityId;
        ...
        public void SelectQuest(QuestListContainer questListContainer)
        {
            ...
            var questListIndex = (EntityUtility.GetEntityInt(questListContainer) == m_questListContainerEntityId.intValue) ? m_questListIndex : 0;
        ...
        public void SelectQuest(Quest quest, QuestListContainer questListContainer, int questListIndex)
        {
            ...
            var questEntityId = EntityUtility.GetEntityId(quest);
            if (quest != null && questEntityId.intValue != m_questEntityId.intValue) m_selectedNodeListIndex = -1;
            m_questEntityId = questEntityId;
            m_questListContainerEntityId = EntityUtility.GetEntityId(questListContainer);
            m_selectedQuestListContainer = questListContainer;
            m_questListIndex = questListIndex;
            m_canvasGUI.AssignQuest(quest);
#if DEBUG_QUEST_EDITOR
            Debug.Log("<color=magenta>QuestEditorWindow.SelectQuest: quest=" + quest + " [instanceID=" + questEntityId.intValue+ ", isAsset=" + 
                ((quest != null) ? (!quest.isInstance).ToString() : "NA") + "], questListContainer=" + questListContainer + 
                " [instanceID=" + m_questListContainerEntityId.intValue+ "], questListIndex=" + questListIndex + "</color>");
#endif
        ...
        private void SelectQuestBasedOnCurrentSelection()
        {
            ...
            else if (m_questListContainerEntityId.isValid) // Can't use current selection, but can use previously-selected QuestListContainer.
            {
                SelectQuest(MoreEditorUtility.InstanceIDToObject(m_questListContainerEntityId) as QuestListContainer);
            }
            else if (m_questEntityId.isValid) // Can't use current selection, but can use previously-selected Quest.
            {
                SelectQuest(MoreEditorUtility.InstanceIDToObject(m_questEntityId) as Quest);
            }
            else
            {
                SelectQuest(null, null, 0);
        ...
        {
            var questListContainer = (m_questListContainerEntityId.isValid) ? MoreEditorUtility.InstanceIDToObject(m_questListContainerEntityId) as QuestListContainer : null;
            if (questListContainer != null && questListContainer.questList != null &&
                ...
                EntityUtility.GetEntityInt(questListContainer.questList[m_questListIndex]) == m_questEntityId.intValue)
            {
        ...
        private void CheckCurrentRuntimeSelection()
        {
            ...
            if (!m_questListContainerEntityId.isValid) return;
            var questListContainer = MoreEditorUtility.InstanceIDToObject(m_questListContainerEntityId) as QuestListContainer;
        ...
        public void DrawNoSelection()
        {
            if (GUILayout.Button(new GUIContent("New quest asset...", "Create a new quest asset file."), GUILayout.Width(160)))
            {
                ...
                {
                    // We need to use names to handle evaluation version,
                    // which doesn't have direct access to Wrappers.
                    var type = System.Type.GetType("PixelCrushers.QuestMachine.Wrappers.Quest, Assembly-CSharp-firstpass");
                    if (type == null)
                    {
                        type = System.Type.GetType("PixelCrushers.QuestMachine.Wrappers.Quest, QuestMachine");
                    }
                    if (type == null)
                    {
                        type = typeof(Quest);
