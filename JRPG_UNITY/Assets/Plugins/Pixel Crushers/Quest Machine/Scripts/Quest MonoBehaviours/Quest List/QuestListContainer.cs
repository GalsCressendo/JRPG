...
namespace PixelCrushers.QuestMachine
{
    ...
    public class QuestListContainer : Saver, IMessageHandler
    {
        ...
        [Tooltip("Use text-based JSON instead of binary to save hand-written quests. (Procedurally-generated quests always use JSON.)\nNOTE: Save data will not be compatible with old saved games that use previous value if you change this value.")]
        [SerializeField]
        private bool m_saveQuestsToJson = false;
        ...
        /// <summary>
        /// Use text-based JSON instead of binary to save hand-written quests. (Procedurally-generated quests always use JSON.)
        /// NOTE: Save data will not be compatible with old saved games that use previous value if you change this value.
        /// </summary>
        public bool saveQuestsToJson
        {
            get { return m_saveQuestsToJson; }
            set { m_saveQuestsToJson = value; }
        }
        ...
        public class SaveData
        {
            ...
            public List<string> staticQuestJsonData = new List<string>();
        ...
        public override string RecordData()
        {
            ...
            for (int i = 0; i < questList.Count; i++)
            {
                ...
                else
                {
                    ...
                    if (saveQuestsToJson)
                    {
                        saveData.staticQuestJsonData.Add(QuestStateSerializer.SerializeJson(quest));
                    }
                    else
                    {
        ...
        public override void ApplyData(string data)
        {
            ...
            try
            {
                ...
                int numStaticQuests = saveQuestsToJson
                    ? Mathf.Min(saveData.staticQuestIds.Count, saveData.staticQuestJsonData.Count)
                    : Mathf.Min(saveData.staticQuestIds.Count, saveData.staticQuestData.Count);
                for (int i = 0; i < numStaticQuests; i++)
                {
                    var questID = saveData.staticQuestIds[i];
                    if (saveQuestsToJson)
                    {
                        var questData = saveData.staticQuestJsonData[i];
                        if (string.IsNullOrEmpty(questID) || string.IsNullOrEmpty(questData)) continue;
                    }
                    else
                    {
                ...
                {
                    var questID = saveData.staticQuestIds[i];
                    ByteData questData = null;
                    string jsonData = null;
                    if (saveQuestsToJson)
                    {
                        jsonData = saveData.staticQuestJsonData[i];
                        if (string.IsNullOrEmpty(questID) || string.IsNullOrEmpty(jsonData)) continue;
                    }
                    else
                    {
                        questData = saveData.staticQuestData[i];
                        if (string.IsNullOrEmpty(questID) || questData == null || questData.bytes == null) continue;
                    }
                    if (deletedStaticQuests.Contains(questID)) continue;
                    ...
                    if (QuestMachine.allowExceptions)
                    {
                        DeserializeInto(quest, saveData, i);
                    }
                    else
                    {
                        try
        ...
        protected virtual void DeserializeInto(Quest quest, SaveData saveData, int i)
        {
            if (saveQuestsToJson)
            {
                var jsonData = saveData.staticQuestJsonData[i];
                QuestStateSerializer.DeserializeJsonInto(quest, jsonData);
            }
            else
            {
                var questData = saveData.staticQuestData[i];
